...
- Use `bun run` package scripts for all validation; do not invoke their
  underlying tools directly. Run `bun run lint`, `bun run test:all`, and any
  package-specific build script before release.
...
- Run `bun run pack:check` before an npm release to inspect the publishable
  contents.

## Release Procedure

1. Update version and package metadata deliberately.
2. Run `bun run lint`, `bun run test:all`, plus any package-specific build
   script.
3. Run `bunx @minifw/tooling prepare`.
4. Run `bun run pack:check`; verify the intended artifacts, `README.md`,
